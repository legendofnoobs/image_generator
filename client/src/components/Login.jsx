		...
		<div className="fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center">
			<form className="relative bg-white p-10 rounded-xl text-slate-500" onSubmit={onSubmitHandler}>
				...
				{state === "Login" ?
					...
					:
					<p className="mt-5 text-center">Alreday have an account <span className="text-blue-600 cursor-pointer" onClick={() => { setState("Login") }}>Login</span></p>
				}
				<img src={assets.cross_icon} alt="close" className=" absolute top-5 right-5 cursor-pointer" onClick={()=>setShowLogin(false)}/>
			</form>
		</div>
	)
}
